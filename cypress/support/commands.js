...
Cypress.Commands.add('login', (
    username = Cypress.env('USER_EMAIL'), // valor default para caso o um user_email não seja passado
    password = Cypress.env('USER_PASSWORD'), // valor default para caso o um user_password não seja passado
    { cacheSession = true } = {}
) => {
    const login = () => {
...
Cypress.Commands.add('createNote', (note, attachFile = false) => {   // recebe uma nota como 1º argumento o no segundo o valor defaul (false) para o attachFile
    cy.visit('/notes/new')
    cy.get('#content').type(note)

    if (attachFile) { // se attachFile = true
        attachFileHandler()
    }
